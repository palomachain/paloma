...
func ModuleBalanceInvariant(k Keeper) sdk.Invariant {
	return func(ctx sdk.Context) (string, bool) {
		...
		for _, actual := range actualBals {
			...
			if err != nil {
				// Here we do not return because a user could halt the chain by gifting skyway a cosmos asset with no erc20 repr
				liblog.FromSDKLogger(k.Logger(sdkCtx)).WithFields("asset", denom).Error("Unexpected skyway module balance of paloma-originated asset with no erc20 representation")
				continue
			}
...
func ValidateStore(ctx sdk.Context, k Keeper) error {
	...
	for _, chain := range chains {
		...
		lastObservedSkywayNonce, err := k.GetLastObservedSkywayNonce(ctx, chain)
		if err != nil {
			return err
		}
		...
		g.Add(
			k.IterateAttestations(ctx, chain, false, func(_ []byte, att types.Attestation) (stop bool) {
				...
				if att.Observed {
					if claim.GetSkywayNonce() > lastObservedSkywayNonce {
						g.Add(fmt.Errorf("last observed event nonce <> observed attestation nonce mismatch (%v < %v)", lastObservedSkywayNonce, claim.GetSkywayNonce()))
						return true
					}
