...
// Tests that the skyway module's balance is accounted for with unbatched txs, including tx cancellation
func TestModuleBalanceUnbatchedTxs(t *testing.T) {
	...
	checkInvariant(t, ctx, input.SkywayKeeper, true)
	...
	err = input.SkywayKeeper.RemoveFromOutgoingPoolAndRefund(ctx, 1, mySender)
	require.NoError(t, err)
	checkInvariant(t, ctx, input.SkywayKeeper, true)
	...
	checkImbalancedModule(t, ctx, input.SkywayKeeper, input.BankKeeper, mySender, sdk.NewCoins(sdk.NewCoin(testDenom, oneVoucher.Amount)))
}

// Tests that the skyway module's balance is accounted for with batches of txs, including unbatched txs and tx cancellation
func TestModuleBalanceBatchedTxs(t *testing.T) {
	...
	batch, err := input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", token.Contract, 3)
	require.NoError(t, err)
	// then check the batch persists
	batch, err = input.SkywayKeeper.GetOutgoingTXBatch(ctx, batch.TokenContract, batch.BatchNonce)
	require.NotNil(t, batch)
	require.NoError(t, err)

	// The module should be balanced with the new unobserved batch + leftover unbatched txs
	checkInvariant(t, ctx, input.SkywayKeeper, true)
	checkImbalancedModule(t, ctx, input.SkywayKeeper, input.BankKeeper, mySender, voucherCoin)

	// Remove a tx from the pool
	require.NoError(t, input.SkywayKeeper.RemoveFromOutgoingPoolAndRefund(ctx, 4, mySender))
	...
	err = input.SkywayKeeper.OutgoingTxBatchExecuted(ctx, batch.TokenContract, msg)
	require.NoError(t, err)
...
func checkImbalancedModule(t *testing.T, ctx context.Context, skywayKeeper Keeper, bankKeeper bankkeeper.BaseKeeper, sender sdk.AccAddress, coins sdk.Coins) {
	// Imbalance the module
	require.NoError(t, bankKeeper.SendCoinsFromAccountToModule(ctx, sender, types.ModuleName, coins))
	checkInvariant(t, ctx, skywayKeeper, false)
