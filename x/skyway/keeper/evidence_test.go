...
func TestSubmitBadSignatureEvidenceBatchExists(t *testing.T) {
	...
	goodBatch, err := input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *tokenContract, 2)
	require.NoError(t, err)
	...
	err = input.SkywayKeeper.CheckBadSignatureEvidence(ctx, &msg, "test-chain")
