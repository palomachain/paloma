...
func TestBatches(t *testing.T) {
	...
	noBatch, err := input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *myTokenContractAddr, 2)
	...
	for i := 0; i < 4; i++ {
		...
		_, err = input.SkywayKeeper.AddToOutgoingPool(ctx, mySender, *myReceiver, amount, "test-chain")
		require.NoError(t, err)
		// Should create:
	...
	err = input.SkywayKeeper.SetLastObservedEthereumBlockHeight(ctx, "test-chain", 1234567)
	require.NoError(t, err)
	// maxElements must be greater than 0, otherwise the batch would not be created
	noBatch, err = input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *myTokenContractAddr, 0)
	require.Nil(t, noBatch)
	require.Error(t, err)

	// tx batch size is 2, so that some of them stay behind
	firstBatch, err := input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *myTokenContractAddr, 2)
	require.NoError(t, err)

	// then batch is persisted
	gotFirstBatch, err := input.SkywayKeeper.GetOutgoingTXBatch(ctx, firstBatch.TokenContract, firstBatch.BatchNonce)
	require.NoError(t, err)
	...
	expFirstBatch := types.OutgoingTxBatch{
		...
		BatchTimeout:       input.SkywayKeeper.getBatchTimeoutHeight(ctx),
	...
	for i, orch := range OrchAddrs {
		...
		_, err = input.SkywayKeeper.SetBatchConfirm(ctx, conf)
		require.NoError(t, err)
	}

	// verify that confirms are persisted
	firstBatchConfirms, err := input.SkywayKeeper.GetBatchConfirmByNonceAndTokenContract(ctx, firstBatch.BatchNonce, firstBatch.TokenContract)
	require.NoError(t, err)
	require.Equal(t, len(OrchAddrs), len(firstBatchConfirms))

	gotUnbatchedTx, err := input.SkywayKeeper.GetUnbatchedTransactionsByContract(ctx, *myTokenContractAddr)
	require.NoError(t, err)
	...
	secondBatch, err := input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *myTokenContractAddr, 2)
	require.NoError(t, err)
	...
	// verify that confirms are persisted
	secondBatchConfirms, err := input.SkywayKeeper.GetBatchConfirmByNonceAndTokenContract(ctx, secondBatch.BatchNonce, secondBatch.TokenContract)
	require.NoError(t, err)
	require.Equal(t, len(OrchAddrs), len(secondBatchConfirms))

	// check that last added batch is the one with the highest nonce
	lastOutgoingBatch, err := input.SkywayKeeper.GetLastOutgoingBatchByTokenType(ctx, *myTokenContractAddr)
	require.NoError(t, err)
	...
	err = input.SkywayKeeper.OutgoingTxBatchExecuted(ctx, secondBatch.TokenContract, msg)
	require.NoError(t, err)

	// check batch has been deleted
	gotSecondBatch, err := input.SkywayKeeper.GetOutgoingTXBatch(ctx, secondBatch.TokenContract, secondBatch.BatchNonce)
	require.NoError(t, err)
...
func TestBatchesFullCoins(t *testing.T) {
	...
	for _, v := range []uint64{20, 300, 25, 10} {
		...
		_, err = input.SkywayKeeper.AddToOutgoingPool(ctx, mySender, *receiverAddr, amount, "test-chain")
		require.NoError(t, err)
	}
	...
	// tx batch size is 2, so that some of them stay behind
	firstBatch, err := input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *tokenContract, 2)
	require.NoError(t, err)
	...
	// and verify remaining available Tx in the pool
	gotUnbatchedTx, err := input.SkywayKeeper.GetUnbatchedTransactionsByContract(ctx, *tokenContract)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	// tx batch size is 2, so that some of them stay behind
	secondBatch, err := input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *tokenContract, 2)
	require.NoError(t, err)
...
//	input.AccountKeeper.NewAccountWithAddress(ctx, mySender)
//	require.NoError(t, input.BankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, mySender, allVouchers))
//	input.SkywayKeeper.SetLastObservedEthereumBlockHeight(ctx, 1234567)
//
//	// CREATE FIRST BATCH
//	// ==================
//
...
//			if (v+1)%100 == 0 {
//				batch, err := input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *contractAddr, 100)
//				require.NoError(t, err)
...
//		require.NoError(t, err)
//		gotBatch := input.SkywayKeeper.GetOutgoingTXBatch(ctx, *contractAddr, batch.BatchNonce)
...
//		now             = time.Now().UTC()
//		mySender, e1    = sdk.AccAddressFromBech32("paloma1ahx7f8wyertuus9r20284ej0asrs085c945jyk")
//		notMySender, e2 = sdk.AccAddressFromBech32("skyway1add7f8wyertuus9r20284ej0asrs085c8ajr0y")
//		myReceiver      = "0xd041c41EA1bf0F006ADBb6d2c9ef9D425dE5eaD7"
...
//	_, err = input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *contract, 2)
//	require.NoError(t, err)
//
//	// try to refund a tx that's in a batch
//	err1 := input.SkywayKeeper.RemoveFromOutgoingPoolAndRefund(ctx, 3, mySender)
//	require.Error(t, err1)
//
//	// try to refund somebody else's tx
//	err2 := input.SkywayKeeper.RemoveFromOutgoingPoolAndRefund(ctx, 2, notMySender)
//	require.Error(t, err2)
//
//	// try to refund a tx that's in the pool
//	err3 := input.SkywayKeeper.RemoveFromOutgoingPoolAndRefund(ctx, 2, mySender)
...
func TestBatchConfirms(t *testing.T) {
	...
	for i := 1; i < 200; i++ {
		...
		// create batch
		_, err = input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *myTokenContractAddr, 1)
		require.NoError(t, err)
	}

	outogoingBatches, err := input.SkywayKeeper.GetOutgoingTxBatches(ctx)
	require.NoError(t, err)
	...
	for _, batch := range outogoingBatches {
		for i, addr := range OrchAddrs {
			batchConfirm, err = input.SkywayKeeper.GetBatchConfirm(ctx, batch.BatchNonce, batch.TokenContract, addr)
			require.NoError(t, err)
...
func TestLastSlashedBatchBlock(t *testing.T) {
	...
	lastSlashedBlock, err := input.SkywayKeeper.GetLastSlashedBatchBlock(ctx)
	assert.NoError(t, err)
	assert.Equal(t, uint64(0), lastSlashedBlock)

	err = input.SkywayKeeper.SetLastSlashedBatchBlock(ctx, 2)
	assert.NoError(t, err)
	...
	err = input.SkywayKeeper.SetLastSlashedBatchBlock(ctx, 1)
	assert.Error(t, err)
	...
	err = input.SkywayKeeper.SetLastSlashedBatchBlock(ctx, 129)
	assert.NoError(t, err)
...
func TestBridgeTax(t *testing.T) {
	...
	// batch should not be created if there is no txs of the given token type in tx pool
	noBatch, err := input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *myTokenContractAddr, 1)
	require.Nil(t, noBatch)
	require.Nil(t, err)
	...
	firstBatch, err := input.SkywayKeeper.BuildOutgoingTXBatch(ctx, "test-chain", *myTokenContractAddr, 1)
	require.NoError(t, err)

	// then batch is persisted
	gotBatch, err := input.SkywayKeeper.GetOutgoingTXBatch(ctx, firstBatch.TokenContract, firstBatch.BatchNonce)
	require.NoError(t, err)
	require.NotNil(t, gotBatch)
