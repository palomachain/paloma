...
func TestAddToOutgoingPool(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		r, err := input.SkywayKeeper.AddToOutgoingPool(ctx, mySender, *receiver, amount, "test-chain")
		require.NoError(t, err)
	...
	// then
	got, err := input.SkywayKeeper.GetUnbatchedTransactionsByContract(ctx, *tokenContract)
	require.NoError(t, err)
...
func TestAddToOutgoingPoolEdgeCases(t *testing.T) {
	...
	r, err = input.SkywayKeeper.AddToOutgoingPool(ctx, mySender, *receiver, badAmount, "test-chain")
	require.Error(t, err)
	require.Zero(t, r)
	...
	mtCoin := new(sdk.Coin)
	r, err = input.SkywayKeeper.AddToOutgoingPool(input.Context, *mtSend, mtRecieve, *mtCoin, "test-chain")
	require.Error(t, err)
	require.Zero(t, r)
}
...
func TestRemoveFromOutgoingPoolAndRefund(t *testing.T) {
	...
	unbatchedTransactions, err := input.SkywayKeeper.GetUnbatchedTransactions(ctx)
	require.NoError(t, err)
...
func TestRemoveFromOutgoingPoolAndRefundCosmosOriginated(t *testing.T) {
	...
	err = input.SkywayKeeper.setDenomToERC20(ctx, "test-chain", myTokenDenom, *tokenAddr)
	require.NoError(t, err)

	addr, err := input.SkywayKeeper.GetERC20OfDenom(ctx, "test-chain", myTokenDenom)
	require.NoError(t, err)
...
func checkRemovedTx(
	...
	err := input.SkywayKeeper.RemoveFromOutgoingPoolAndRefund(ctx, id, mySender)
	require.NoError(t, err)
	*amountSum -= amount // user should have regained the locked amounts from tx
	...
	err = input.SkywayKeeper.IterateUnbatchedTransactions(ctx, func(key []byte, tx *types.InternalOutgoingTransferTx) bool {
...
func TestRefundInconsistentTx(t *testing.T) {
	...
	err = input.SkywayKeeper.addUnbatchedTX(ctx, &types.InternalOutgoingTransferTx{
		Id:          uint64(5),
		Sender:      mySender,
	...
	err = input.SkywayKeeper.RemoveFromOutgoingPoolAndRefund(ctx, uint64(5), mySender)
	require.Error(t, err)
...
func TestRefundNonexistentTx(t *testing.T) {
	...
	origBalances := input.BankKeeper.GetAllBalances(ctx, mySender)
	err := input.SkywayKeeper.RemoveFromOutgoingPoolAndRefund(ctx, uint64(1), mySender)
	require.Error(t, err)
	newBalances := input.BankKeeper.GetAllBalances(ctx, mySender)
	require.Equal(t, origBalances, newBalances)
}
...
func TestRefundTwice(t *testing.T) {
	...
	txId, err := input.SkywayKeeper.AddToOutgoingPool(ctx, mySender, *receiver, sdk.NewCoin(testDenom, amountToken.Amount), "test-chain")
	require.NoError(t, err)
	afterAddBalances := input.BankKeeper.GetAllBalances(ctx, mySender)

	// First refund goes through
	err = input.SkywayKeeper.RemoveFromOutgoingPoolAndRefund(ctx, txId, mySender)
	require.NoError(t, err)
...
func TestGetUnbatchedTransactions(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		r, err := input.SkywayKeeper.AddToOutgoingPool(ctx, mySender, *receiver, amount1, "test-chain")
		require.NoError(t, err)
		ids[i] = r
	...
	require.NoError(t, err)
	tokenId := ids[0]
	tx, err := input.SkywayKeeper.GetUnbatchedTxByAmountAndId(ctx, *tokenAmount, tokenId)
	require.NoError(t, err)
	...
	// GetUnbatchedTxById
	tx, err = input.SkywayKeeper.GetUnbatchedTxById(ctx, tokenId)
	require.NoError(t, err)
	require.Equal(t, *expTx, *tx)

	// GetUnbatchedTransactionsByContract
	tokenTxs, err := input.SkywayKeeper.GetUnbatchedTransactionsByContract(ctx, *tokenContract1)
	require.NoError(t, err)
	...
	allTxs, err := input.SkywayKeeper.GetUnbatchedTransactions(ctx)
	require.NoError(t, err)
...
func TestIterateUnbatchedTransactions(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		require.NoError(t, err)
		r, err := input.SkywayKeeper.AddToOutgoingPool(ctx, mySender, *receiver, sdk.NewCoin(testDenom, amount1.Amount), "test-chain")
		require.NoError(t, err)
		ids[i] = r
		idToTxMap[r] = &types.OutgoingTransferTx{
			Id:          r,
	...
	err = input.SkywayKeeper.IterateUnbatchedTransactionsByContract(ctx, *tokenContract1, func(key []byte, tx *types.InternalOutgoingTransferTx) bool {
		require.NotNil(t, tx)
		fTx := idToTxMap[tx.Id]
...
func TestTransferLimits(t *testing.T) {
	...
	err = input.SkywayKeeper.SetBridgeTransferLimit(ctx, &types.BridgeTransferLimit{
		Token:       testDenom,
