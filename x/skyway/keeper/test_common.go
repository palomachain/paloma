...
import (
	...
	consensuskeeper "github.com/palomachain/paloma/v2/x/consensus/keeper"
	consensustypes "github.com/palomachain/paloma/v2/x/consensus/types"
	"github.com/palomachain/paloma/v2/x/evm"
	evmkeeper "github.com/palomachain/paloma/v2/x/evm/keeper"
	...
	"github.com/palomachain/paloma/v2/x/treasury"
...
func addValidators(t *testing.T, input *TestInput, count int) {
	...
	for i, addr := range ValAddrs {
		...
		require.NoError(t, err)
		valAddress, err := utilkeeper.ValAddressFromBech32(input.SkywayKeeper.AddressCodec, validator.GetOperator())
		if err != nil {
			require.NoError(t, err)
		}
...
func CreateTestEnv(t *testing.T) TestInput {
	...
	skywayKey := storetypes.NewKVStoreKey(types.StoreKey)
	...
	ms.MountStoreWithDB(skywayKey, storetypes.StoreTypeIAVL, db)
	...
	*evmKeeper = *evmkeeper.NewKeeper(
	...
	// set skywayIDs for batches and tx items, simulating genesis setup
	err = k.setLastObservedSkywayNonce(ctx, "test-chain", 0)
	require.NoError(t, err)
	...
	k.SetParams(ctx, TestingSkywayParams)
...
func (t TestInput) AssertInvariants() {
	...
	gravInvariantFunc := AllInvariants(t.SkywayKeeper)
