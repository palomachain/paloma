...
func TestStoreIDs(t *testing.T) {
	...
	t.Run("generating next increases the ID by one", func(t *testing.T) {
		...
		assert.Equal(t, uint64(1), k.ider.incrementNextID(ctx, keyName))
		assert.Equal(t, uint64(2), k.ider.incrementNextID(ctx, keyName))
		assert.Equal(t, uint64(3), k.ider.incrementNextID(ctx, keyName))
	})

	t.Run("calling lastID with empty storage returns zero", func(t *testing.T) {
		k, ctx := newSchedulerKeeper(t)
		assert.Equal(t, uint64(0), k.ider.getLastID(ctx, keyName))
	})

	t.Run("calling lastID after incrementing returns the same value that was inserted", func(t *testing.T) {
		k, ctx := newSchedulerKeeper(t)
		assert.Equal(t, uint64(1), k.ider.incrementNextID(ctx, keyName))
		assert.Equal(t, uint64(1), k.ider.getLastID(ctx, keyName))
	})
	...
	t.Run("test that ID names are not coupled", func(t *testing.T) {
		...
		k.ider.incrementNextID(ctx, name1)
		k.ider.incrementNextID(ctx, name1)
		k.ider.incrementNextID(ctx, name1)

		// and increase it once for name2
		k.ider.incrementNextID(ctx, name2)

		assert.Equal(t, uint64(3), k.ider.getLastID(ctx, name1))
		assert.Equal(t, uint64(1), k.ider.getLastID(ctx, name2))
	})
