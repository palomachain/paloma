...
// ExecuteJob provides a mock function with given fields: ctx, definition, payload, senderAddress, contractAddress, refID
func (_m *Bridge) ExecuteJob(ctx types.Context, definition []byte, payload []byte, senderAddress types.AccAddress, contractAddress types.AccAddress, refID string) error {
	ret := _m.Called(ctx, definition, payload, senderAddress, contractAddress, refID)

	var r0 error
	if rf, ok := ret.Get(0).(func(types.Context, []byte, []byte, types.AccAddress, types.AccAddress, string) error); ok {
		r0 = rf(ctx, definition, payload, senderAddress, contractAddress, refID)
	} else {
		r0 = ret.Error(0)
	}
