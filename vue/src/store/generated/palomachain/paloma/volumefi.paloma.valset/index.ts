...
import { ValidatorExternalAccounts } from "./module/types/valset/snapshot"
import { Snapshot } from "./module/types/valset/snapshot"
import { ExternalChainInfo } from "./module/types/valset/snapshot"


export { Params, Validator, ValidatorExternalAccounts, Snapshot, ExternalChainInfo };
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						...
						ValidatorExternalAccounts: getStructure(ValidatorExternalAccounts.fromPartial({})),
...
export default {
	...
	getters: {
			...
			return state.ValidatorInfo[JSON.stringify(params)] ?? {}
				...
				getGetSnapshotByID: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.GetSnapshotByID[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		...
		async QueryGetSnapshotByID({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryGetSnapshotById( key.snapshotId)).data
				
					
				commit('QUERY', { query: 'GetSnapshotByID', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryGetSnapshotByID', payload: { options: { all }, params: {...key},query }})
				return getters['getGetSnapshotByID']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryGetSnapshotByID API Node Unavailable. Could not perform query: ' + e.message)
