...
export interface ValidatorExternalAccounts {
  address: Uint8Array;
...
export const Validator = {
  encode(message: Validator, writer: Writer = Writer.create()): Writer {
    ...
    for (const v of message.externalChainInfos) {
      ExternalChainInfo.encode(v!, writer.uint32(26).fork()).ldelim();
    }
...
const baseValidatorExternalAccounts: object = {};

export const ValidatorExternalAccounts = {
  encode(
    ...
    for (const v of message.ExternalChainInfo) {
  ...
  decode(
    ...
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = {
      ...baseValidatorExternalAccounts,
    } as ValidatorExternalAccounts;
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          break;
        case 2:
          message.ExternalChainInfo.push(
            ExternalChainInfo.decode(reader, reader.uint32())
          );
          break;
        default:
  ...
  fromJSON(object: any): ValidatorExternalAccounts {
    const message = {
      ...baseValidatorExternalAccounts,
    } as ValidatorExternalAccounts;
    message.ExternalChainInfo = [];
    ...
    if (
      object.ExternalChainInfo !== undefined &&
      object.ExternalChainInfo !== null
    ) {
      for (const e of object.ExternalChainInfo) {
        message.ExternalChainInfo.push(ExternalChainInfo.fromJSON(e));
      }
    }
    return message;
  },

  toJSON(message: ValidatorExternalAccounts): unknown {
    const obj: any = {};
    message.address !== undefined &&
      (obj.address = base64FromBytes(
        message.address !== undefined ? message.address : new Uint8Array()
      ));
    if (message.ExternalChainInfo) {
      obj.ExternalChainInfo = message.ExternalChainInfo.map((e) =>
        e ? ExternalChainInfo.toJSON(e) : undefined
      );
    } else {
      obj.ExternalChainInfo = [];
    }
    return obj;
  },

  fromPartial(
    object: DeepPartial<ValidatorExternalAccounts>
  ): ValidatorExternalAccounts {
    const message = {
      ...baseValidatorExternalAccounts,
    } as ValidatorExternalAccounts;
    ...
    if (
      ...
      for (const e of object.ExternalChainInfo) {
        message.ExternalChainInfo.push(ExternalChainInfo.fromPartial(e));
      }
    }
    return message;
  },
...
const baseSnapshot: object = { id: 0, height: 0 };
...
const baseExternalChainInfo: object = {
  chainType: "",
  chainID: "",
  address: "",
};
